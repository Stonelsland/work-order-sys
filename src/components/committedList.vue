...
<template>
   <div class="mainMenu">
      ...
      <Table border :columns="orderColumns" :data="committedList">
   ...
   export default {
      data() {
         return {
            ...
            orderList: store.state.committedList,
            stateList: store.state.stateList,
         }
      },
      methods: {
         orderDetail(title, approver, business, host, state, content, creator, time, index) {
            this.$router.push({
               path: `/orderDetail`,
               query: {
                  title,
